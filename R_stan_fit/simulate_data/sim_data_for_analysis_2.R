...
results_nt_ed = read.table(Sys.glob(file.path('fitted_params/2/', "*ED_NT*.csv")),sep=",",header=T)
results_tr_ed = read.table(Sys.glob(file.path('fitted_params/2/', "*ED_BID*.csv")),sep=",",header=T)
results_nt_hc = read.table(Sys.glob(file.path('fitted_params/2/', "*HC_NT*.csv")),sep=",",header=T)
results_tr_hc = read.table(Sys.glob(file.path('fitted_params/2/', "*HC_BID*.csv")),sep=",",header=T)

thetas_winfo=list(results_nt_ed,results_tr_ed,results_nt_hc,results_tr_hc)
output_converted_all = NULL
params_filename = str_replace(str_replace(paste(Sys.glob(file.path('fitted_params/2/', "*ED_NT*.csv")),'_plus',sep=''),'.csv',''),'fitted_params/2//','')
