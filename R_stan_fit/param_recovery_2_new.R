...
param_sum_filename = 'fitted_ind_params_summary_2021_09_13_21_22_38_niter_10_ED_NT_foerde_alter_2_JO_wrong_ship.csv'

fitted_par_sum <- read.csv(paste("output/params/CSV/2/",param_sum_filename,sep=''), header = T)
...
for (seed in 1:length(seeds)){
  ...
  if (save_outputs){
    write.csv(simulated,file=paste("output/recovery/2/","recovery_sim_params_seed_",seeds[seed],'_','itern_',n_iter,'_',time_run,'_from',param_sum_filename,'.csv',sep=''),row.names=FALSE)
    write.csv(fitted,file=paste("output/recovery/2/","recovery_fit_params_seed_",seeds[seed],'_','itern_',n_iter,'_',time_run,'_from',param_sum_filename,'.csv',sep=''),row.names=FALSE)
    write.csv(thetas_corr[seed,],file=paste("output/recovery/2/","recovery_corr_seed_",seeds[seed],'_','itern_',n_iter,'_',time_run,'_from',param_sum_filename,'.csv',sep=''),row.names=FALSE)
  }
...
if (save_outputs){
  ...
  write.csv(thetas_corr,file=paste("output/recovery/2/","recovery_corr_all_",time_saved,'_seedn_',seed_num,'itern_',n_iter,'_from',param_sum_filename,'.csv',sep=''),row.names=FALSE)
  write.csv(theta_corr_summary,file=paste("output/recovery/2/","recovery_corr_summary",time_saved,'_seedn_',seed_num,'itern_',n_iter,'_from',param_sum_filename,'.csv',sep=''),row.names=FALSE)
}
