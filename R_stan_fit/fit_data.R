...
for (i in sets){
  ...
  if (save_outputs){ # iters>=1000 && 
    saveRDS(HBA_ind_fit,file=paste("output/HB_fits/",date.time.append("HBA_ind_fit"),'_niter_',iters,'_',list_names[i],'_',model_name_short,'.rds',sep=''))
  ...
  if (save_outputs){
    saveRDS(fitted_ind_params,file=paste("output/params/RDS/",date.time.append("fitted_ind_params"),'_','niter_',iters,'_',list_names[i],'_',model_name_short,'.rds',sep=''))
    saveRDS(fitted_params_ind_summary,file=paste("output/params/RDS/",date.time.append("fitted_params_ind_summary"),'_','niter_',iters,'_',list_names[i],'_',model_name_short,'.rds',sep=''))
    write.csv(fitted_params_ind_summary_df,file=paste("output/params/CSV/",date.time.append("fitted_ind_params_summary"),'_','niter_',iters,'_',list_names[i],'_',model_name_short,'.csv',sep=''),row.names=FALSE)
    write.csv(fitted_ind_params_df,file=paste("output/params/CSV/",date.time.append("fitted_ind_params_df"),'_',list_names[i],'_','niter_',iters,'_',model_name_short,'.csv',sep=''),row.names=FALSE)
