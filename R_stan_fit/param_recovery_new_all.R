...
for (f in 1:length(param_sum_filenames)){
  ...
  for (seed in 1:length(seeds)){
    ...
    if (save_outputs){
      write.csv(simulated,file=paste("output/recovery/all/","recovery_sim_params_seed_",seeds[seed],'_','itern_',n_iter,'_',time_run,'_from',param_sum_filenames[f],'.csv',sep=''),row.names=FALSE)
      write.csv(fitted,file=paste("output/recovery/all/","recovery_fit_params_seed_",seeds[seed],'_','itern_',n_iter,'_',time_run,'_from',param_sum_filenames[f],'.csv',sep=''),row.names=FALSE)
      write.csv(thetas_corr[seed,],file=paste("output/recovery/all/","recovery_corr_seed_",seeds[seed],'_','itern_',n_iter,'_',time_run,'_from',param_sum_filenames[f],'.csv',sep=''),row.names=FALSE)
    }
  ...
  if (save_outputs){
    ...
    write.csv(thetas_corr,file=paste("output/recovery/all/","recovery_corr_all_",time_saved,'_seedn_',seed_num,'itern_',n_iter,'_from',param_sum_filenames[f],'.csv',sep=''),row.names=FALSE)
    write.csv(theta_corr_summary,file=paste("output/recovery/all/","recovery_corr_summary",time_saved,'_seedn_',seed_num,'itern_',n_iter,'_from',param_sum_filenames[f],'.csv',sep=''),row.names=FALSE)
  }
