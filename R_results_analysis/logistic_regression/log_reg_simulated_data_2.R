...
load_recoded=TRUE
saveCSV=FALSE
file_name_load = "simulated_data_from_fitted_ind_params_df_2021_09_13_23_15_27_ED_NT_niter_4000_foerde_alter_2_JO_wrong_ship_plus"

# Shift data ---------------------------------
if(shiftMe){
  data_all = read.table(paste('data/simulated/2/',file_name_load,".csv",sep=""),sep=",",header=T)
  shift<-function(x,k){
  ...
  if(saveCSV){
    write.csv(data_all_shifted,paste('data/simulated/2/',file_name_load,'_shifted.csv',sep=''),row.names = FALSE)
  }
}

# Recode data ---------------------------------
if (recodeMe || shiftMe){
  data_all_shifted = read.table(paste('data/simulated/2/',file_name_load,'_shifted.csv',sep=''),sep=",",header=T)
  ...
  if(saveCSV){
    write.csv(data_all_shifted_recoded,paste('data/simulated/2/',file_name_load,'_shifted_recoded.csv',sep=''),row.names = FALSE)
  }
}

# Load data ---------------------------------
if(load_recoded && !(recodeMe || shiftMe)){
  data_all_shifted_recoded = read.table(paste('data/simulated/2/',file_name_load,"_shifted_recoded.csv",sep=''),sep=",",header=T)
}
...
summary(model1)
if (save_outputs){
  sink("model_summaries/simulated_data/2/model1_summary_simulated.txt")
