setwd("~/Google Drive/Academia/PostUoE_Man/manuscript_v2/R_analysis/log reg/simulated_data/")
library(lme4)
library(jtools)
library(tidyverse)
library(useful)
...
if(shiftMe){
  ...
  data_all = read.table(paste(file_name_load,".csv",sep=""),sep=",",header=T)
  shift<-function(x,k){
  ...
  if(saveCSV){
    write.csv(data_all_shifted,paste(file_name_load,'shifted.csv',sep='_'),row.names = FALSE)
  }
}

# Recode data ---------------------------------
if (recodeMe || shiftMe){
  data_all_shifted = read.table(paste(file_name_load,'shifted.csv',sep='_'),sep=",",header=T)
  ...
  if(saveCSV){
    ...
    write.csv(data_all_shifted_recoded,paste(file_name_load,'shifted_recoded.csv',sep='_'),row.names = FALSE)
  }
}

# Load data ---------------------------------
if(load_recoded && !(recodeMe || shiftMe)){
  data_all_shifted_recoded = read.table(paste(file_name_load,"_shifted_recoded.csv",sep=''),sep=",",header=T)
}
...
model1_2 <- glmer(stay~r*transition*(group)+(r*transition+1|sub), family = binomial, data=data_all_shifted_recoded,control = glmerControl(optimizer = "bobyqa",optCtrl = list(maxfun=1e5)))
summary(model1_2)
